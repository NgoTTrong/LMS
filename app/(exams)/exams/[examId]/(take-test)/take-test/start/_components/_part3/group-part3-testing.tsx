...
const GroupPart3Testing = ({ part3Question }: Props) => {
	const {
		exam,
		pushResult,
		result,
	...
	const handleNext = () => {
		const questionIdx =
			exam?.part3?.part3Questions?.findIndex((question) =>
				question?.groupPart3Questions.some(
					(e) => e?.questionId == currentQuestion
				)
			) ?? -1;
		if (questionIdx == (exam?.part3?.part3Questions?.length ?? 0) - 1) {
			const nextPart = getNextPart(exam!, 3);
			const nextPartQuestion = getNextPartQuestion(exam!, 3);
			setCurrentPart(nextPart);
			setCurrentQuestion(nextPartQuestion);
		} else {
			setCurrentQuestion(
				exam?.part3?.part3Questions?.[questionIdx + 1]
					?.groupPart3Questions?.[0]?.questionId ?? null
			);
		}
		window.scrollTo({
			top: 0,
	...
	const handlePrev = () => {
		...
		if (questionIdx != 0 && questionIdx != -1) {
			...
			if (exam) {
				setCurrentPart(getPrevPart(exam, 3));
				setCurrentQuestion(getPrevPartQuestion(exam, 3));
			}
		}
		window.scrollTo({
			top: 0,
	...
	return (
		<main className="flex flex-col gap-6 items-end">
			{part3Question?.groupPart3Questions?.map((group, idx) => {
				return (
					<form
						key={"group-part-3-" + idx}
...
export default GroupPart3Testing;
