...
const AnswerPart2 = ({ part2Questions, history, exam }: Props) => {
	const [numOfCorrects, setNumOfCorrects] = useState<number>(0);
	const [extend, setExtend] = useState<boolean>(false);
	const [openDetail, setOpenDetail] = useState<boolean>(false);
	const [choosenQuestion, setChoosenQuestion] = useState<Part2Question>();
	const checkQuestion = (
		questionId: string,
		answer: "A" | "B" | "C" | "D"
	) => {
			...
			<section className="flex flex-col gap-6 p-4 rounded-lg shadow-lg w-full">
				...
				>
					<h1 className="text-lg font-medium flex items-center gap-1">
						Part 2 ({numOfCorrects} / {part2Questions?.length}){" "}
						{numOfCorrects == part2Questions?.length ? (
				...
				{extend && (
					<div className="flex flex-col gap-6">
						{part2Questions?.map((question, idx) => (
							<div
