...
type Props = {
	items: IPart1Question[];
	onReorder: (updateData: { id: string; position: number }[]) => void;
	onEdit: (id: string) => void;
};
const QuestionsList = ({ items, onReorder, onEdit }: Props) => {
	const [isMounted, setIsMounted] = useState<boolean>(false);
	const [questions, setQuestions] = useState<IPart1Question[]>(items);
	...
	const handleDeleteQuestion = async (part1QuestionId: string) => {
		try {
			const _part1QuestionId = await Part1Service.deleteQuestion(
				part1QuestionId
			);
			if (_part1QuestionId) {
