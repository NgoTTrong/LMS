...
const QuestionsForm = ({ initialData, part6Id }: Props) => {
    ...
    const onReorder = async (
        ...
        try {
            setIsUpdating(true);
            const _update = await Part6Service.reorderQuestions(
                part6Id,
                updateData
            );
            if (_update) {
        ...
        <section className="mt-6 bg-slate-100 rounded-md p-4 relative">
            ...
            <div className="font-medium flex items-center justify-between">
                ...
                {isCreating ? (
                    ...
                    <ModalAddQuestion part6Id={part6Id}>
                        <>
            ...
            {!isCreating && (
                <div
                    className={cn(
                        "text-sm mt-2",
                        !initialData?.part6Questions?.length &&
                            "text-slate-500 italic"
                    )}
                >
                    {initialData?.part6Questions?.length == 0 && "No questions"}
                    <QuestionsList
                        onEdit={onEdit}
                        onReorder={onReorder}
                        items={initialData?.part6Questions ?? []}
                    />
                </div>
            )}
            {!isCreating && (
