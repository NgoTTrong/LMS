...
import {
    IMappingPart6Question,
    IPart6Question,
} from "@/interfaces/part-6/part-6-interface";
...
export function ModalEditQuestion({ question, children }: Props) {
    ...
    useEffect(() => {
        ...
        setQuestions(question?.groupPart6Questions?.map((e) => e.question));
    }, [question]);
    const handleEditQuestion = async () => {
        try {
            setIsLoading(true);
            const _questionPart6 = await Part6Service.updateQuestion(
                question?.id,
                questions,
                imageUrl
            );
    ...
    return (
        <Dialog
            onOpenChange={(open) => {
                if (!open) {
                    setQuestions(
                        question?.groupPart6Questions?.map((e) => e.question)
                    );
                }
            }}
        >
