...
const QuestionsForm = ({ initialData, part1Id }: Props) => {
    ...
    const onReorder = async (
        ...
        try {
            setIsUpdating(true);
            const _update = await Part1Service.reorderQuestions(
                part1Id,
                updateData
            );
            if (_update) {
        ...
        <section className="mt-6 bg-slate-100 rounded-md p-4 relative">
            ...
            {!isCreating && (
                <div
                    className={cn(
                        "text-sm mt-2",
                        !initialData?.part1Questions?.length &&
                            "text-slate-500 italic"
                    )}
                >
                    {initialData?.part1Questions?.length == 0 && "No questions"}
                    <QuestionsList
                        onEdit={onEdit}
                        onReorder={onReorder}
                        items={initialData?.part1Questions ?? []}
                    />
                </div>
            )}
            {!isCreating && (
