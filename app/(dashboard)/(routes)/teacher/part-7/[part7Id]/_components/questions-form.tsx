...
const QuestionsForm = ({ initialData, part7Id }: Props) => {
    ...
    const onReorder = async (
        ...
        try {
            setIsUpdating(true);
            const _update = await Part7Service.reorderQuestions(
                part7Id,
                updateData
            );
            if (_update) {
        ...
        <section className="mt-6 bg-slate-100 rounded-md p-4 relative">
            ...
            <div className="font-medium flex items-center justify-between">
                ...
                {isCreating ? (
                    ...
                    <ModalAddQuestion part7Id={part7Id}>
                        <>
            ...
            {!isCreating && (
                <div
                    className={cn(
                        "text-sm mt-2",
                        !initialData?.part7Questions?.length &&
                            "text-slate-500 italic"
                    )}
                >
                    {initialData?.part7Questions?.length == 0 && "No questions"}
                    <QuestionsList
                        onEdit={onEdit}
                        onReorder={onReorder}
                        items={initialData?.part7Questions ?? []}
                    />
                </div>
            )}
            {!isCreating && (
