...
const QuestionsForm = ({ initialData, part5Id }: Props) => {
    ...
    const onReorder = async (
        ...
        try {
            setIsUpdating(true);
            const _update = await Part5Service.reorderQuestions(
                part5Id,
                updateData
            );
            if (_update) {
        ...
        <section className="mt-6 bg-slate-100 rounded-md p-4 relative">
            ...
            <div className="font-medium flex items-center justify-between">
                ...
                {isCreating ? (
                    ...
                    <ModalAddQuestion part5Id={part5Id}>
                        <>
            ...
            {!isCreating && (
                <div
                    className={cn(
                        "text-sm mt-2",
                        !initialData?.part5Questions?.length &&
                            "text-slate-500 italic"
                    )}
                >
                    {initialData?.part5Questions?.length == 0 && "No questions"}
                    <QuestionsList
                        onEdit={onEdit}
                        onReorder={onReorder}
                        items={initialData?.part5Questions ?? []}
                    />
                </div>
            )}
            {!isCreating && (
