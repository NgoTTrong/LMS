...
import {
    IMappingPart4Question,
    IPart4Question,
} from "@/interfaces/part-4/part-4-interface";
...
export function ModalEditQuestion({ question, children }: Props) {
    ...
    useEffect(() => {
        ...
        setQuestions(question?.groupPart4Questions?.map((e) => e.question));
    }, [question]);
    const handleEditQuestion = async () => {
        try {
            setIsLoading(true);
            const _questionPart4 = await Part4Service.updateQuestion(
                question?.id,
                questions,
                imageUrl,
                audioUrl
    ...
    return (
        <Dialog
            onOpenChange={(open) => {
                if (!open) {
                    setQuestions(
                        question?.groupPart4Questions?.map((e) => e.question)
                    );
                }
            }}
        >
