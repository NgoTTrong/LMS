...
import { IPart4Question } from "@/interfaces/part-4/part-4-interface";
...
type Props = {
    items: IPart4Question[];
    onReorder: (updateData: { id: string; position: number }[]) => void;
    onEdit: (id: string) => void;
};
const QuestionsList = ({ items, onReorder, onEdit }: Props) => {
    const [isMounted, setIsMounted] = useState<boolean>(false);
    const [questions, setQuestions] = useState<IPart4Question[]>(items);
    ...
    const handleDeleteGroup = async (part4QuestionId: string) => {
        try {
            const _part4QuestionId = await Part4Service.deleteQuestion(
                part4QuestionId
            );
            if (_part4QuestionId) {
