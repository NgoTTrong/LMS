...
const QuestionsForm = ({ initialData, part4Id }: Props) => {
    ...
    const onReorder = async (
        ...
        try {
            setIsUpdating(true);
            const _update = await Part4Service.reorderQuestions(
                part4Id,
                updateData
            );
            if (_update) {
        ...
        <section className="mt-6 bg-slate-100 rounded-md p-4 relative">
            ...
            <div className="font-medium flex items-center justify-between">
                ...
                {isCreating ? (
                    ...
                    <ModalAddQuestion part4Id={part4Id}>
                        <>
            ...
            {!isCreating && (
                <div
                    className={cn(
                        "text-sm mt-2",
                        !initialData?.part4Questions?.length &&
                            "text-slate-500 italic"
                    )}
                >
                    {initialData?.part4Questions?.length == 0 && "No questions"}
                    <QuestionsList
                        onEdit={onEdit}
                        onReorder={onReorder}
                        items={initialData?.part4Questions ?? []}
                    />
                </div>
            )}
            {!isCreating && (
